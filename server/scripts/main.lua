...
-- this is being done as rgb instances aren't being sent through networking
...
local function ServerMessage(text, player)
    if player then
        teverse.networking:sendToClient(player, "message", text, ServerColour)
    else
        teverse.networking:broadcast("message", text, ServerColour)
    end
end
...
teverse.networking:on("message", function(client, message, targetPlayers)
    -- i apologise in advance for this messy code
